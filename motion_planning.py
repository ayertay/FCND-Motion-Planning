...
class MotionPlanning(Drone):
    ...
    def plan_path(self):
        ...
        lat0 = float(re.findall(r"[-+]?\d*\.\d+|\d+", row[0])[1])
        lon0 = float(re.findall(r"[-+]?\d*\.\d+|\d+", row[1])[1])
        ...
        grid_start = (
            int(np.ceil(current_position[0] - north_offset)),
            int(np.ceil(current_position[1] - east_offset)),
        )
        ...
        print(
            "Start_north = {0}, Start_east = {1}".format(grid_start[0], grid_start[1])
        )
